...
configurar_servidor() {
    ...
    # Añadir el archivo /etc/rsyslog.d/10-remot.conf para que los mensajes recibidos de los clientes se guarden en directorios separados por máquina y fecha
    cat <<EOT > /etc/rsyslog.d/10-remot.conf
    $template GuardaRemots, "/var/log/remots/%HOSTNAME%/%timegenerated:1:10:daterfc3339%"
